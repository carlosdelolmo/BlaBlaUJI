...
public class Viaje implements Serializable {
    ...
    /**
     *
     * Contructor con los atributos de la clase, salvo el codviaje, que se contruye
    ...
    public Viaje(String codprop, String origen, String destino, String fecha, long precio, long numplazas) {
        ...
        this.codviaje = this.construyeCodviaje().toLowerCase();
    ...
    public Viaje(JSONObject jsonViaje) {
        ...
        JSONArray array =  (JSONArray) jsonViaje.get("pasajeros");
        for(int i = 0; i< array.size(); i++)
            p.add((String) array.get(i));
        this.pasajeros = p;
        this.codviaje = this.construyeCodviaje().toLowerCase();
    }
    ...
    public JSONObject toJSON() {
        ...
        obj.put("codviaje", this.codviaje);
        ...
        for (String pasajero : getPasajeros())
            array.add(pasajero);
        obj.put("pasajeros", array);
    ...
    public boolean finalizado() {
        ...
        return dia.isBefore(hoy) || dia.equals(hoy);
    ...
    public boolean anyadePasajero(String pasajero) {
        if (quedanPlazas() && !codprop.toLowerCase().equals(pasajero.toLowerCase()) && !pasajeros.contains(pasajero.toLowerCase())) {
            pasajeros.add(pasajero.toLowerCase());
            numplazas--;
            return true;
        }
        return false;
    ...
    public boolean borraPasajero(String pasajero) {
        if (pasajeros.contains(pasajero.toLowerCase())) {
            pasajeros.remove(pasajero.toLowerCase());
            numplazas++;
            return true;
        }
        return false;
